...
def auditory_plot(fp):
    ...
    for song in songs:
        fig, axes = plt.subplots(len(conditions)+2,1,sharex='all',figsize=(5,5))
        ...
        for i in range(len(conditions)):
            ...
            axes[i*2+1].plot(raster,y,'|',color="black")
            axes[i*2+1].set_xlim([xmin,xmax])
            axes[i*2+1].set_ylim([0,10])
            axes[i*2+1].set_title(conditions[i])
            axes[i*2+1].get_xaxis().set_tick_params(direction='out')
            axes[i*2+1].get_yaxis().set_tick_params(direction='out')
        #fig.tight_layout()
